...
function gameRecord(){
    ...
    for(var i=0; i< record[0].length; i++) {
        $("#data").append(`
            <tr>
                <td class= "user"><img src="./static/image/${record[0][1]}"> </td>
                <td class="com"><img src="./static/image/${record[1][i]}"> </td>
                <td class="outcome">${record[0][1]} </td>
            `)
    }
...
function startAndStop(){
    ...
    if( $(this).text() === '종료'){
        ...
        $(".userImg").off('click'); // 종료시 유저 가위바위보 크릭 이벤트 해제
...
function userSelect(){
    ...
    // userimg 클래스들을 배열로 가져오고 그중에서 몇 번째 인덱스 클릭했냐?
    $(this).css('border', '1px solid red'); // 내가 클릭한 가위바위보 이미지 표시
    clearInterval(comHd); // 컴퓨터의 가위바위보 이미지 변경 하는 거 멈추기(clearInterval)
...
function comStart(){
    ...
    }, 100); // 지정된 시간에 한 번씩 실행, 시간 단위는 밀리세컨드 1000이 1초
}

function localSave(){

    // JSON.stringify() JSON 이라는 문자열로 변환시켜준다.
    // 숫자 -> 문자열 10+""

    let recordJSON = JSON.stringify(record);
    localStorage.setItem("record", recordJSON);

    // localStorage.setItem("name", "banana"); // localStorage - 문자열 데이터 저장
}